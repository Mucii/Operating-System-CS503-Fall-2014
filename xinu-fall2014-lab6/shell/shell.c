	...
	)				/*   to accept commands		*/
{
	...
	char	*args[SHELL_MAXTOK];	/* Argument vector passed to	*/
					/*   builtin commands		*/
	void	*dynamic_cmd;		/* Pointer to a dynamically 	*/
					/*   loaded command		*/
	char	*dynamic_cmd_name;	/* Name of the dynamically 	*/
					/*   loaded command		*/
	...
	while (TRUE) {
		...
		if (j >= ncmd) {
			...
			dynamic_cmd_name = tokbuf;
		}

		/* Handle built-in command */

		if ((j < ncmd) && (cmdtab[j].cbuiltin)) { /* No background or redirect. */
		...
		if (j < ncmd) {
			...
			child = create(dynamic_cmd,
				SHELL_CMDSTK, SHELL_CMDPRIO,
				dynamic_cmd_name, 2, ntok, &tmparg);
		}
